...
In each dataset, a record relates to an enforcement event, not an individual. Individuals may be involved in multiple enforcement events of each category. Unique individual identifiers (A-numbers, `alien_file_number`) are redacted in the original datasets as released by ICE; it is not possible to link events related to a specific individual, nor is it possible to link a given encounter to an associated arrest, or an arrest to a subsequent removal.[^1]
...
[^3]: An additional "HQ" AOR does not have an associated territorial jurisdiction. Some ICE publications and datasets also refer to a "HAL" AOR for the south Texas region centering on Harlingen, TX, which overlaps with the HOU and SNA regions as displayed in this notebook.
