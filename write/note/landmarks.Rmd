...
The [Encounters](encounters.html) dataset contains `r nrow(enc %>% distinct(landmark))` distinct `landmark` values; or `r nrow(enc %>% distinct(landmark, aor))` distinct combinations of `aor` and `landmark`.
...
The [Arrests](arrests.html) dataset contains `r nrow(arr %>% distinct(apprehension_landmark))` distinct `apprehension_landmark` values; or `r nrow(arr %>% distinct(apprehension_landmark, aor))` distinct combinations of `aor` and `apprehension_landmark`.
