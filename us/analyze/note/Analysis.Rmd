...
arrests <- arrests %>% mutate(apprehension_landmark2 = 
                                case_when(grepl('NON_SPECIFIC|GENERAL AREA|OPERATIONS|FUGITIVE', apprehension_landmark) ~ 0,
                                          apprehension_landmark %in% c('ATLANTA, GA', 'SAN ANGELO SUB-OFFICE', 'BALTIMORE CITY MD', 'NORTHERN VIRGINIA AREA', 'SEA CAP', 'DETROIT, MI', 'DO NOT USE', '')  ~ 0,
                                          TRUE ~ 1))
#test <- arrests %>% select(apprehension_landmark, apprehension_landmark2)
test2 <- arrests %>% filter(apprehension_landmark2 == 1) %>%
  group_by(apprehension_landmark) %>% 
  summarize(total = n())

specificity <- arrests %>% 
  group_by(fy, aor, apprehension_landmark2) %>%
  summarize(total = n()) %>% 
  pivot_wider(names_from = fy, values_from = total)
  
specificity %>% kable(caption = 'Apprehension landmark specificity by AOR FY2016-2019 (1: Specific, 0: Non-specific)')
```
...
encounters <- encounters %>% mutate(landmark2 = 
                                case_when(grepl('NON_SPECIFIC|GENERAL AREA|OPERATIONS|FUGITIVE', landmark) ~ 0,
                                          landmark %in% c('DO NOT USE', 'PHILADELPHIA - SECURE COMMUNITIES', 'LOGANVILLE, GA', '287g', '')  ~ 0,
                                          TRUE ~ 1))
test <- encounters %>% select(landmark, landmark2)
test2 <- encounters %>% filter(landmark2 == 1) %>%
  group_by(landmark) %>% 
  summarize(total = n())

specificity2 <- encounters %>% 
  group_by(fy, aor, landmark2) %>%
  summarize(total = n()) %>% 
  pivot_wider(names_from = fy, values_from = total)
  
specificity2 %>% kable(caption = 'Encounter landmark specificity by AOR FY2016-2019 (1: Specific, 0: Non-specific)')
```
