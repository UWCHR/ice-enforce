...
dat_map <- dat %>%
  mutate(fy = as.numeric(as.character(fy))) %>% 
  full_join(demog, arr_fy_aor, by=c('aor' = 'aor', 'fy' = 'year')) %>% 
  group_by(aor) %>% 
  fill(contains('pop')) %>% 
  left_join(aor_geom, by='aor') %>% 
  filter(
...
p1 <- arr %>% 
  count(year_mth, arrest_method_short) %>% 
  ggplot(aes(x=year_mth, y=n, color=arrest_method_short)) +
  geom_line() +
  labs(title = "ICE arrests by 'arrest_method', FY12-22",
       subtitle = "All categories above >20k total arrests")

p1

p2 <- arr %>% 
  count(year_mth, arrest_method) %>% 
  filter(arrest_method %in% c("CAP Local Incarceration",
                                    "CAP State Incarceration",
                                    "CAP Federal Incarceration")) %>% 
  ggplot(aes(x=year_mth, y=n, color=arrest_method)) +
  geom_line() +
  labs(title = "ICE arrests by 'arrest_method', FY12-22",
       subtitle = "Criminal Alien Program (CAP)")
...
p3 <- arr %>% 
  ...
  geom_line() +
  labs(title = "ICE arrests by 'arrest_method', FY12-22",
       subtitle = "Non-Custodial Arrests")

p3

```

```{r national_method_pct_change}
...
natl_pct_chg <- arr %>%
  filter(arrest_method %in% c("CAP Local Incarceration",
                              ...
                              "CAP Federal Incarceration")) %>% 
  filter(arrest_date <= "2022-09-30") %>% 
  group_by(arrest_method, fy) %>%
  summarize(n = n()) %>% 
  mutate(pct_change = (n/lag(n) - 1))

p1 <- natl_pct_chg %>% 
  ggplot(aes(x = fy, y = pct_change, fill = arrest_method)) +
  geom_col(position = "dodge") +
  scale_y_continuous(labels = scales::percent) +
  labs(title="FY % change in CAP arrests")

p1

# These numbers seem different than calculated elsewhere
...
aor_pct_chg <- arr %>%
  ...
  filter(arrest_method %in% c("CAP Local Incarceration",
                              "CAP State Incarceration",
                              "CAP Federal Incarceration")) %>% 
  group_by(arrest_method, aor, fy) %>%
  summarize(n = n()) %>% 
  group_by(arrest_method, aor) %>% 
  arrange(fy, .by_group=TRUE) %>% 
  mutate(pct_change = (n/lag(n) - 1))
...
p2 <- aor_pct_chg %>% 
  ...
  scale_x_discrete(breaks=seq(2012, 2022, 2)) +
  facet_wrap(~aor)  +
  labs(title="FY % change in CAP arrests per AOR") +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
