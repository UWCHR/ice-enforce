...
keyword_comparison <- function(keyword, aor_to_compare) {
  ...
  p2 <- dat %>% 
    ...
    geom_line() +
    labs(title = paste0("% of keyword '", keyword, "' in `apprehension_landmark` per FY"),
         subtitle = paste0(aor_to_compare, " versus National")) +
    scale_x_discrete(breaks=seq(2012, 2022, 4)) +
    ylim(0, NA) +
    facet_wrap(~dataset) +
...
# write_delim(sea_landmarks, here('us-fy12-23', 'analyze', 'output', paste0(specific_aor, "_landmarks.csv")), delim=',')

```

```{r analyze_sea_sea_landmarks}

sea_landmarks <- read_delim(here('us-fy12-23', 'analyze', 'hand', "SEA_landmarks.csv"), delim='|')
