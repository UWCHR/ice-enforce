...
sea_enc <- enc %>% 
  filter(aor == "SEA") %>% 
  mutate(landmark = str_squish(toupper(landmark))) %>% 
  left_join(., ldmrks, by=c("landmark"="apprehension_landmark"))
...
p3 <- sea_enc %>% 
  ...
  group_by(fy, landmark_type) %>% 
  summarize(n = n()) %>% 
  ggplot(aes(x = fy, y=n, fill=landmark_type, color = landmark_type)) +
  geom_col() +
  labs(title = "Total ICE encounters by landmark type",
       subtitle = "Seattle Area of Resonsibility")
