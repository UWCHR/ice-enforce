...
p2 <- sea_arr %>% 
  ...
  ggplot(aes(x=fy, y=n, color=landmark_type, fill=landmark_type, group=landmark_type)) +
  geom_col() +
  labs(title="Apprehension landmark categories, SEA AOR",
...
p4 <- sea_arr %>% 
  filter(arrest_date >= '2016-10-01',
         ...
         landmark_state %in% c('WA', 'OR'),
         arrest_method_short == "CAP Federal Incarceration") %>% 
  count(year_mth, landmark_state, arrest_method_short) %>% 
  ggplot(aes(x=year_mth, y=n, color=arrest_method_short, fill=arrest_method_short, group=arrest_method_short)) +
  geom_col() +
