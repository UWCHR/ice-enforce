...
# write_delim(ldmrks, here('us-fy12-23', 'analyze', 'output', paste0(specific_aor, "_landmarks.csv")), delim=',')

```

```{r analyze_sea_ldmrks}

ldmrks <- read_delim(here('us-fy12-23', 'analyze', 'hand', "SEA_landmarks.csv"), delim='|')

sea_arr <- arr %>% 
  filter(aor == "SEA") %>% 
  mutate(apprehension_landmark = str_squish(toupper(apprehension_landmark))) %>% 
  left_join(., ldmrks, by="apprehension_landmark")

p1 <- sea_arr %>% 
  filter(arrest_date >= '2016-10-30') %>% 
...
```{r output_plots, include=FALSE, message=FALSE, warning=FALSE}

specific_aor <- "SEA"

methods <- arr %>% 
  filter(aor == specific_aor) %>% 
  count(arrest_method_short) %>% 
  filter(n > 50) %>% 
  arrange(arrest_method_short)

pdf(here('us-fy12-23', 'analyze', 'output', paste0(specific_aor, "_method_trends.pdf")),width=6,height=3.25)

for (i in 1:length(methods$arrest_method_short)) {
  current_method <- as.character(methods$arrest_method_short[i])

  p1 <- arr %>% 
    filter(!is.na(aor),
           arrest_date >= '2016-10-30',
           arrest_method_short == current_method) %>%
    count(aor, year_mth, arrest_method_short) %>%
    ggplot(aes(x = year_mth, y = n, color=aor, group=aor)) +
    geom_line() +
    gghighlight(aor == specific_aor,
...
landmarks <- landmarks %>% 
  mutate(
    landmark_type = case_when(
      ...
      str_detect(value, "NON-DETAINED") ~ "ICE OPERATIONAL",
    ...
    nondetaineddocket = case_when(
      str_detect(value, "NON-DETAINED DOCKET") ~ TRUE,
                 TRUE ~ FALSE)
    )
...
p1 <- dat %>% 
  count(fy, nondetaineddocket) %>% 
  ggplot(aes(x = fy, y = n, fill = nondetaineddocket, color = nondetaineddocket)) +
  geom_col(position="fill")
