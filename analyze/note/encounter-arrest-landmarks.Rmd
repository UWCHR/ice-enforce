...
enc <- enc %>% 
  mutate(event_date = as_date(event_date, format="%m/%d/%Y"),
         ...
         fy = as.factor(substr(quarter(event_date, fiscal_start=10, type="year.quarter"), 1,4)),
...
sea_arr <- arr %>% 
  filter(aor == "SEA",
         arrest_date >= as.Date("2019-05-21")) %>% 
  count(apprehension_landmark) %>% 
...
sea_enc <- enc %>% 
  filter(aor == "SEA",
         event_date >= as.Date("2019-05-21")) %>% 
  count(landmark) %>% 
  mutate(type = "encounter")

dat <- rbind(sea_arr, sea_enc)

dat <- complete(dat, landmark, type, fill = list(n = 0))

tab <- dat %>% 
  group_by(landmark, type) %>% 
  summarize(n = sum(n)) %>% 
  pivot_wider(names_from = type, values_from = n) %>% 
  ungroup() %>% 
  arrange(desc(arrest)) %>% 
  mutate(landmark_rank = row_number(),
         total_landmarks = max(landmark_rank),
         rank = paste0(landmark_rank, "/", total_landmarks)) %>% 
  filter(str_detect(landmark, "JAIL|COUNTY|CO\\.")) %>% 
  dplyr::select(-c(landmark_rank, total_landmarks))
...
sea_arr <- arr %>% 
  filter(aor == "SEA") %>% 
  count(fy, apprehension_landmark) %>% 
  rename(landmark = apprehension_landmark) %>% 
  mutate(type = "arrest")
...
dat <- complete(dat, landmark, type, fy, fill = list(n = 0)) %>% 
  filter(!is.na(fy))

p1 <- dat %>% 
  filter(landmark == "CLARK COUNTY JAIL") %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "CLARK COUNTY JAIL")

p1

p2 <- dat %>% 
  filter(landmark == "GRANT CO. JAIL, WA") %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "GRANT CO. JAIL, WA")

p2

p3 <- dat %>% 
  filter(landmark == "ADAMS CO. JAIL") %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "ADAMS CO. JAIL")

p3

p4 <- dat %>% 
  filter(str_detect(landmark, "BENTON CO. JAIL")) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "BENTON CO. JAIL")

p4

p5 <- dat %>% 
  filter(str_detect(landmark, "KING COUNTY JAIL")) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "KING COUNTY JAIL")

p5

p6 <- dat %>% 
  filter(str_detect(landmark, "WHATCOM COUNTY JAIL")) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "WHATCOM COUNTY JAIL")

p6

p7 <- dat %>% 
  filter(str_detect(landmark, "KENT CITY JAIL")) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "KENT CITY JAIL")

p7

p8 <- dat %>% 
  filter(str_detect(landmark, "SCORE")) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  geom_vline(aes(xintercept = 2019), color = "black") +
  scale_x_discrete(breaks=levels(dat$fy)) %>% 
  labs(title = "SCORE (SOUTH CORRECTIONAL ENTITY)")

p8

```

```{r multiple_landmark_enc_arr}

selections <- c("CLARK COUNTY JAIL", "GRANT CO. JAIL, WA", "ADAMS CO. JAIL", "KING COUNTY JAIL", "WHATCOM COUNTY JAIL", "KENT CITY JAIL")

p9 <- dat %>% 
  mutate(fy = as.numeric(as.character(fy))) %>% 
  filter(landmark %in% selections) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  geom_vline(aes(xintercept = 2019.5), linetype = "dashed") +
  scale_x_continuous(breaks=seq(2012, 2022, 4)) +
  facet_wrap(~landmark, ncol=2, scales = "free_y") +
  labs(title = "ICE encounters and arrests, FY2012-22",
       subtitle = "Selected `landmark` & `apprehension_landmark` values",
       caption = "Source: ICE ERO LESA\nFigure: UW Center for Human Rights")

p9

selections <- c("SEATTLE NON-DETAINED DOCKET", "POO CAP NON-CUSTODIAL ARREST", "SEA CAP", "SEATTLE FUG OPS", "PORTLAND FUGITIVE OPERATIONS STREET ARREST", "SEATTLE FUGITIVE OPERATIONS STREET ARREST", "EUGENE NON-DETAINED DOCKET", "YAKIMA FUG OPS")

p10 <- dat %>% 
  mutate(fy = as.numeric(as.character(fy))) %>% 
  mutate(operational_ldmk = case_when(
    str_detect(landmark, "NON-DETAINED|NON DETAINED") ~ "NON-DETAINED DOCKET",
    str_detect(landmark, "FUGITIVE|FUG OPS|STREET ARREST") ~ "FUG OPS/STREET ARREST",
    str_detect(landmark, "CAP") ~ "CAP AT-LARGE",
    TRUE ~ "ALL OTHER"
    )) %>%
  group_by(fy, operational_ldmk, type) %>% 
  summarize(n = sum(n)) %>% 
  ggplot(aes(x = fy, y = n, color = type, fill = type, group = type)) + 
  geom_line() +
  geom_vline(aes(xintercept = 2019.5), linetype = "dashed") +
  scale_x_continuous(breaks=seq(2012, 2022, 4)) +
  facet_wrap(~operational_ldmk, ncol=2, scales = "fixed") +
  labs(title = "ICE encounters and arrests, FY2012-22",
       subtitle = "Selected `landmark` & `apprehension_landmark` categories",
       caption = "Source: ICE ERO LESA\nFigure: UW Center for Human Rights")
...
p1 <- sea_arr %>% 
  filter(arrest_method == "Non-Custodial Arrest") %>% 
  count(year_mth, apprehension_landmark) %>% 
  ggplot(aes(x = year_mth, y = n, fill = apprehension_landmark))
