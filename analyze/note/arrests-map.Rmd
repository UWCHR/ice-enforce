...
p_load(here, tidyverse, zoo, lubridate, ggplot2, plotly, gghighlight, sf, tigris, ggrepel, ggmap, tidygeocoder, scales)
...
p1 <- dat %>%
  filter(as.numeric(as.character(fy)) > 2012) %>% 
  ggplot(aes(geometry=geometry, fill=pct_change)) +
  geom_sf() +
  theme_void() +
  labs(title = "% change in total ICE arrests, FY2012-2022") +
  facet_wrap(~fy)
...
p1 <- dat %>%
  ...
  theme_void() +
  labs(title = "% change in total CAP Local Incarceration arrests, FY2012-2022") +
  facet_wrap(~fy)
...
aor_nca_pct_chg <- arr %>%
  filter(arrest_date <= "2022-09-30",
         !is.na(aor),
         arrest_method == "Non-Custodial Arrest") %>% 
  group_by(fy, aor) %>%
  summarize(n = n()) %>% 
  group_by(aor) %>% 
  arrange(fy, .by_group=TRUE) %>% 
...
p0 <- dat %>%
  ...
  theme_void() +
  labs(title = "Total Non-Custodial Arrests, FY2012-2022") +
  facet_wrap(~fy)
...
p1 <- dat %>%
  ...
  scale_fill_viridis_c(labels=percent) +
  theme_void() +
  labs(title = "% change in total Non-Custodial Arrests, FY2012-2022") +
  facet_wrap(~fy)
