...
top_landmarks <- c("Local jail", "State prison", "Federal prison", "At-large", "Non-detained docket", "Detained docket", NA_character_)

sea_arr <- arr %>% 
  filter(aor == "SEA") %>% 
  mutate(apprehension_landmark = str_squish(toupper(apprehension_landmark))) %>% 
  left_join(., sea_landmarks, by="apprehension_landmark") %>% 
  mutate(landmark_type = case_when(landmark_type %in% top_landmarks ~ landmark_type,
                                           TRUE ~ "All others"))
...
p1 <- sea_arr %>% 
  ...
  labs(title="Apprehension landmark categories, SEA AOR",
       ...
       caption = "`landmark_type` inferred from `apprehension_landmark` values.")
